...
plot_hybrid_distrib = function(df_plot, names_var,yat = seq(0,1,l=5), ylab = c("0","0.25","0.5","0.75","1"),ysize = 1.4,yname = "Quantiles", cex_legend = 1){ #plot hybrid mixture
  ...
  legend("right", legend=(df_plot[seq(2, nrow(df_plot), by=2),1]), fill=color_palette, inset = c(-0.18,0),title="Weights",cex=cex_legend) #legend with weights
